...
export function GridScan({
  ...
  return (
    <div ref={containerRef} className={`re-gridscan-bg ${className ?? ""}`} style={style}>
      {showPreview && (
        <div className="re-gridscan-preview">
          <video ref={videoRef} muted playsInline autoPlay className="re-gridscan-video" />
          <div className="re-gridscan-status">
...
function dist2(a: { x: number; y: number }, b: { x: number; y: number }) {
  return Math.hypot(a.x - a.x, b.y - b.y);
}
