...
export function renderExecutiveInquiryHtml(input: ExecutiveInquiryEmailInput): string {
  ...
  const adminBaseUrl = (process.env.ADMIN_ORIGIN || process.env.ADMIN_PUBLIC_BASE_URL || "https://admin.ractysh.com").replace(/\/+$/, "");
            ...
            <tr>
              <td align="center" style="padding:30px 42px 36px;border-top:1px solid #eadfc9;background:#ffffff">
                ...
                ${divisions.map((division) => {
                  ...
                  return `<p style="margin:0;color:#655a4b;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:22px"><a href="${escapeHtml(url)}" style="color:#655a4b;text-decoration:none">${escapeHtml(division)}</a></p>`;
                }).join("")}
                <a href="https://www.ractysh.com" style="display:inline-block;margin-top:18px;color:#7a0f16;font-family:Arial,Helvetica,sans-serif;font-size:12px;font-weight:700;text-decoration:none">www.ractysh.com</a>
              </td>
            </tr>
          </table>
        </td>
